...
@click.option('--num_crops', type=int, help='Number of crops to generate for each image')
@click.option('--crop_size', type=int,  default=128, help='Crop size in pixels')
@click.option('--out_dir', type=click.Path(exists=False, path_type=Path), default=Path("./crops"), help='Output path')
    ...
    with tqdm(enumerate(files), unit="image", total=len(files)) as tq:
        for idx, f in tq:
            ...
            with Image.open(f.as_posix()) as im:
                ...
                im_1000 = im.resize((math.floor(resized_to*im.size[0]/im.size[1]),resized_to))
