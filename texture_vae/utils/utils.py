...
def create_variations(autoencoder, count, latent_dims, image_size:int, channels:int, img=None):
    z = torch.distributions.Normal(0,1).sample((count, latent_dims)).to(autoencoder.device)
    img = autoencoder.decoder(z)
    imgs = (img.reshape(count, channels,image_size,image_size).detach().to('cpu').permute(0,2,3,1).numpy()*255).astype('uint8')
    return imgs
